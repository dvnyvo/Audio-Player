...
public class AudioBook extends AudioContent
{
  ...
  // Print information about the audiobook. First print the basic information of the AudioContent 
	...
	{
		super.printInfo();
		System.out.println("Author: "+this.author+" Narrated by: "+this.narrator);		//prints general info then the specifics of audiobook
	}
  ...
  // Play the audiobook by setting the audioFile to the current chapter title (from chapterTitles array list) 
	...
	public void play()
	{
		setAudioFile(chapterTitles.get(currentChapter)+"\n"+chapters.get(currentChapter));	//sets audiofile to chapTitle and chapter at currenchap then play
		super.play();
	}
	...
	public void printTOC()
	{
		for(int i=0;i<chapterTitles.size();i++){
			System.out.println("Chapter "+(i+1)+". "+chapterTitles.get(i));	// prints each chapter of a book by looping through the chapter titles
		}
	}
	...
	public void selectChapter(int chapter)
	{
		...
		{
			currentChapter = chapter - 1;						//if chapter is valid set current chapter to chapter-1
		}	
	}
	...
	{
		AudioBook otherabook = (AudioBook) other;
		return (this.getAuthor() == otherabook.getAuthor())&&(this.getNarrator() == otherabook.getNarrator());  //casts other to Audiobook to compare by auther and narrator
	}
