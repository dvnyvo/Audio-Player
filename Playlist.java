...
public class Playlist
{
	...
	{
		for(int i=0;i<contents.size();i++){			//loops through contents then prints out indexes followed by content's info
	...
	public void playAll()
	{
		for(int i=0;i<contents.size();i++){
			contents.get(i).printInfo();			//loops through contents then prints their infos and plays them
			contents.get(i).play();
		}
	}
	...
	public void play(int index)
	{
		if(contents.contains(index)){
			contents.get(index-1).play();				//plays content at given index-1
		}
	}
	
	public boolean contains(int index)
	{
		return index >= 1 && index <= contents.size();	//checks if index is valid
	}
	
	// Two Playlists are equal if their titles are equal
	public boolean equals(Object other)
	{
		Playlist other_playlist = (Playlist) other;

		return this.getTitle().equals(other_playlist.getTitle());		//casts other to playlist to compare titles
	}
	...
	{
		if (!contains(index)) return;									//if the index is not valid return false
		contents.remove(index-1);										//else remove from contents at given index-1
	}
