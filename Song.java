...
public class Song extends AudioContent implements Comparable<Song>// implement the Comparable interface
{
	...
	{
		super.printInfo();
		System.out.println("Artist: "+ this.getArtist()+" Composer: "+ this.getComposer()+" Genre: "+this.getGenre()); //prints generic info than specifics of song
	}
	...
	{
		AudioContent other_content = (AudioContent) other;
		if(this.getTitle().equals(other_content.getTitle())){
			Song other_song = (Song) other;
			
			return (this.getArtist().equals(other_song.getArtist()))&&(this.getComposer().equals(other_song.getComposer())); //cast object to a song to compare artist and composer
		}
		return false;
	}
	...
	{
		return this.getTitle().compareTo(other.getTitle()); //compares the titles
	}
}
